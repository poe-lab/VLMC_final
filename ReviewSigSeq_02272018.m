function ReviewSigSeq_02272018
...
for i = 1:numSpindles
    ...
    startTime = EschenkoSpindle.timestamp(i) - floor(addTime/2);
    stopTime = EschenkoSpindle.timestamp(i) + EschenkoSpindle.duration(i) + ceil(addTime/2);
    ...
    if ~isempty(spikesInSpindle)
        ...
        ax1 = subplot(3,1,1);
        targetIdx = HcTimeStamps >= startTime & HcTimeStamps <= stopTime;
        plot(ax1, HcTimeStamps(targetIdx), HcSpindleSignal(targetIdx), 'b')
        hold on
        
        plot(ax1, HcTimeStamps(targetIdx), HcSignal(targetIdx),...
            'Color', [0.5 0.5 0.5])
        
        targetIdx = HcTimeStamps >= EschenkoSpindle.timestamp(i) &...
            HcTimeStamps <= (EschenkoSpindle.timestamp(i)+EschenkoSpindle.duration(i));
        plot(ax1, HcTimeStamps(targetIdx),...
            HcSpindleSignal(targetIdx), 'g')
        
        targetIdx = rippleFiltTS >= startTime & rippleFiltTS <= stopTime;
...
%         ax2.TickDir = 'out';
        ...
        ax3 = subplot(3,1,3);
        targetIdx = ctxTimeStamps >= startTime & ctxTimeStamps <= stopTime;
        
        plot(ax3, ctxTimeStamps(targetIdx), ctxSpindleSignal(targetIdx), 'b')
        hold on
        plot(ax3, ctxTimeStamps(targetIdx), ctxSignal(targetIdx), 'Color', [0.5 0.5 0.5])
        targetIdx = ctxTimeStamps >= EschenkoSpindle.timestamp(i) &...
            ctxTimeStamps <= (EschenkoSpindle.timestamp(i)+EschenkoSpindle.duration(i));
        
        plot(ax3, ctxTimeStamps(targetIdx),...
